...
local aa = {
    function()
        ...
        -- ===== WINDUI ELEMENTS: Divider & Space =====
        z["AddDivider"] = function(C, D)
            local _div = v.New("Frame", {
            ...
            if C and C.Container then _div.Parent = C.Container end
            return {Frame = _div, Type = "Divider", Destroy = function() _div:Destroy() end}
        end
        z["AddSpace"] = function(C, D)
            local _h = (D and type(D) == "table" and D.Height) or 8
            local _sp = v.New("Frame", {
                Size = UDim2.new(1, 0, 0, _h),
                BackgroundTransparency = 1,
                BorderSizePixel = 0,
                LayoutOrder = 7,
            })
            if C and C.Container then _sp.Parent = C.Container end
            return {Frame = _sp, Type = "Space", Destroy = function() _sp:Destroy() end}
        end
        ...
        function x.SetTheme(C, D)
            if x.Window and (table.find(x.Themes, D) or (type(D)=="string" and type(e(o.Themes)[D])=="table")) then
                x.Theme = D
                p.UpdateTheme()
                -- Apply IconColor and IconSize from theme if defined
        ...
        -- ===== CUSTOM THEME SYSTEM =====
        -- Usage: Fluent:RegisterCustomTheme("MyTheme", { Accent = ..., AcrylicMain = ..., ... })
        ...
        -- Legacy: Fluent.Themes["MyTheme"] = theme; Fluent:SetTheme("MyTheme")
        -- The Themes table is accessible as x.Themes (Names list) and e(o.Themes) for data

        -- ===== NEW MANAGER SYSTEM =====
        -- Managers activate automatically when SetLibrary is called
        -- No need to store: local SaveManager = Fluent.SaveManager etc.
        -- Just call: SaveManager:SetLibrary(Fluent) -- SaveManager is global via x.SaveManager
        -- The managers are accessible as Fluent.SaveManager, Fluent.InterfaceManager, etc.
        -- When SetLibrary is called, managers auto-register themselves
        local _origSMSetLib = x.SaveManager and x.SaveManager.SetLibrary
        local _origIMSetLib = x.InterfaceManager and x.InterfaceManager.SetLibrary
        local _origFBSetLib = x.FloatingButtonManager and x.FloatingButtonManager.SetLibrary
        if x.SaveManager then
            local _sm = x.SaveManager
            function _sm:SetLibrary(lib)
                self.Library = lib
                self.Options = lib.Options
                -- Auto-expose to global for convenience
                if getgenv then pcall(function() getgenv().SaveManager = self end) end
            end
        end
        if x.InterfaceManager then
            local _im = x.InterfaceManager
            function _im:SetLibrary(lib)
                self.Library = lib
                if getgenv then pcall(function() getgenv().InterfaceManager = self end) end
            end
        end
        if x.FloatingButtonManager then
            local _fb = x.FloatingButtonManager
            function _fb:SetLibrary(lib)
                self.Library = lib
                if getgenv then pcall(function() getgenv().FloatingButtonManager = self end) end
            end
        end
...
do local ab,ac,ad,ae,af,ag,ah,aj,c,e,f,g,h,i,j,k=task,setmetatable,...
